...
def find_frame(hs, h, w):
    ...
    size = tuple((p_max - p_min).astype(int))
...
def process(img):
    ...
    R = np.linalg.multi_dot((Rotation.from_euler('x', x_rotation).as_matrix(),
                             Rotation.from_euler('z', -z_rotation).as_matrix()))
    H = np.linalg.multi_dot((K, R, Ki))
    h, w, _ = img.shape
    size, ht = find_frame([H], h, w)

    res04 = cv2.warpPerspective(img, ht.dot(H), size)
    cv2.imwrite("res04.jpg", res04)
